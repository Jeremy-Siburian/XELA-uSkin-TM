...
class Robotiq(object):
	...
	def read_command(self, register_address, data_length):
		data = bytearray()
	...
	def status_thread(self, name): # a thread for reading the gripper status	
		while True:
	...
	def initialize_status_thread(self): # to activate the thread
		status_thread = threading.Thread(target=self.status_thread, args=(1,)) #A thread for accessing uSkin data from the middleware
		status_thread.start()
		

	def write_command(self, args): # write a message to Robotiq gripper
		data = bytearray()
		data.append(SLAVE_ID)
	...
	def home(self):
		...
		self.write_command([WRITE_REGISTER_1, COMMAND_1, COMMAND_2,COMMAND_3])
	...
	def stop(self):
		...
		self.write_command([WRITE_REGISTER_1, COMMAND_1])             		
			
	def position(self):
		self.ser.reset_input_buffer()
		self.read_command(READ_REGISTER_3, 1)          		
		data = self.ser.read(7)
		if len(data)<5:
			return float("nan"),float("nan")
		return data[3], data[4]*10
