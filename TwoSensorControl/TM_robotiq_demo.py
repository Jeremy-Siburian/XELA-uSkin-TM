...
if __name__ == "__main__":
    ...
    def activate_gripper():
        print("Current gripper position: ", gripper.position()[0])
        time.sleep(1)
    ...
    async def grasping():
        ...
        print("Initial grasp force reached.")
        time.sleep(1)
    ...
    async def pick_pos(conn):
        ...
        await conn.move_to_joint_angles_ptp([12.633, 31.263, 108.309, -49.372, 88.583, 14.925], 1.0, 200)
        await conn.set_queue_tag(1, wait_for_completion=True)

    async def waypoint_1(conn):
        await conn.move_to_joint_angles_ptp([12.969, 14.555, 97.971, -22.319, 88.504, 15.313], 0.7, 200)

    async def waypoint_2(conn):
        await conn.move_to_joint_angles_ptp([-19.610, 20.141, 90.185, -21.389, 88.503, 15.283], 1.0, 200)

    async def place_pos(conn):
        await conn.move_to_joint_angles_ptp([-19.610, 27.032, 97.442, -35.536, 88.540, 15.250], 1.0, 200)
